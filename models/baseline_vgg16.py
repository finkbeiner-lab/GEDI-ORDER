"""Copied for reference from original GEDI repo"""
...
class model_struct:
    ...
    def get_conv_var(
        ...
        if init_type == 'xavier':
            ...
            weight_init = tf.random.truncated_normal(
                [filter_size, filter_size, in_channels, out_channels],
                0.0, 0.001)
        bias_init = tf.random.truncated_normal([out_channels], .0, .001, seed=0)
    ...
    def get_fc_var(self, in_size, out_size, name, init_type='xavier'):
        ...
        bias_init = tf.random.truncated_normal([out_size], .0, .001, seed=0)
    ...
    def get_var(
        ...
        if self.trainable:
            # Variable, change the boolian to numpy
            if type(value) is list:
                var = tf.Variable(
                    name=var_name, shape=value[0], initial_value=value[1])
            else:
                var = tf.Variable(name=var_name, initial_value=value)
        else:
...
if __name__ == '__main__':
    ...
    vgg = model_struct(vgg16_npy_path=p.vgg16_weight_path)
    vgg.build(imgs,output_shape=2)
    tf_fun.softmax_cost()
    cost = tf_fun.softmax_cost(vgg.fc8, lbls)
    other_opt_vars, ft_opt_vars = tf_fun.fine_tune_prepare_layers(
        tf.trainable_variables(), p.fine_tune_layers)
    train_op = tf_fun.ft_non_optimized(
        cost, other_opt_vars, ft_opt_vars,
        tf.train.AdamOptimizer, p.hold_lr, p.new_lr)