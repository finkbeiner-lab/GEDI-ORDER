...
class Record:
    ...
    def __init__(self, images_dir_live, images_dir_dead, tfrecord_dir, split, scramble):
        ...
        if not scramble:  #SCRAMPLE SHOULD BE FALSE!!!
...
if __name__ == '__main__':
    p = param.Param()
    pos_dir = '/Volumes/My Passport for Mac/DNA-ML/Testing/Negative'
    neg_dir = '/Volumes/My Passport for Mac/DNA-ML/Testing/Positive'
    split = [.7, .15, .15]

    Rec = Record(pos_dir, neg_dir, p.tfrecord_dir, split, scramble=False)
    savetrain = os.path.join(p.tfrecord_dir, 'test_train.tfrecord')
    saveval = os.path.join(p.tfrecord_dir, 'test_val.tfrecord')
    savetest = os.path.join(p.tfrecord_dir, 'test_test.tfrecord')
