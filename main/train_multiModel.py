...
class Train:
    ...
    def run(self, pos_dirs, neg_dirs, balance_method='cutoff'):
        ...
        # Get all timepoints and train a separate model for each
        pos_ims_dict, neg_ims_dict, timepoints = self.gather_imgs(pos_dirs, neg_dirs, filetype='tif')

        print(f"\nWill train {len(timepoints)} separate models, one for each timepoint:")
        for tp in timepoints:
            print(f"  {tp}: {len(pos_ims_dict[tp])} live, {len(neg_ims_dict[tp])} dead images")

        # Train one model per timepoint
        for timepoint in timepoints:
            if len(pos_ims_dict[timepoint]) == 0 or len(neg_ims_dict[timepoint]) == 0:
                print(f"\nSkipping {timepoint} - insufficient data")
                continue

            print(f"\n{'='*60}")
            print(f"Training model for timepoint: {timepoint}")
            print(f"{'='*60}")

            # Create timepoint-specific directory
            timepoint_dir = os.path.join(self.parent_dir, f"timepoint_{timepoint}")
            os.makedirs(timepoint_dir, exist_ok=True)

            # Generate tfrecords for this timepoint only
            self.generate_tfrecs_for_timepoint(pos_ims_dict[timepoint], neg_ims_dict[timepoint],
                                             timepoint_dir, balance_method, timepoint)

            # Train model for this timepoint
            self.train_timepoint_model(timepoint_dir, timepoint)
    ...
    def generate_tfrecs_for_timepoint(self, pos_ims_list, neg_ims_list, tfrec_dir, balance_method, timepoint):
        """Generate tfrecords for a specific timepoint"""
        print(f"\nGenerating tfrecords for {timepoint}:")
        print(f"  Positive images: {len(pos_ims_list)}")
        print(f"  Negative images: {len(neg_ims_list)}")

        if len(pos_ims_list) == 0 or len(neg_ims_list) == 0:
            raise ValueError(f"Insufficient data for timepoint {timepoint}")

        split = [.7, .15, .15]
        Rec = Record(pos_ims_list, neg_ims_list, tfrec_dir, split, balance_method, split_method='percentage')

        # Save tfrecords with timepoint-specific names
        ...
        print(f'Saved tfrecords for {timepoint} to {tfrec_dir}')

    def train_timepoint_model(self, timepoint_dir, timepoint):
        """Train a model for a specific timepoint"""
        print(f'\nTraining model for {timepoint}...')
        tf.keras.backend.clear_session()

        # Setup filepaths
        data_train = os.path.join(timepoint_dir, 'train.tfrecord')
        data_val = os.path.join(timepoint_dir, 'val.tfrecord')
        data_test = os.path.join(timepoint_dir, 'test.tfrecord')

        assert os.path.exists(data_train), f'Training tfrecord not found: {data_train}'

        timestamp = update_timestring()
        model_name = f"{self.p.which_model}_{timepoint}_{timestamp}"

        # Create directories for this timepoint if they don't exist
        models_dir = os.path.join(self.p.models_dir, f"timepoint_{timepoint}")
        ckpt_dir = os.path.join(self.p.ckpt_dir, f"timepoint_{timepoint}")
        run_info_dir = os.path.join(self.p.run_info_dir, f"timepoint_{timepoint}")
        tb_log_dir = os.path.join(self.p.tb_log_dir, f"timepoint_{timepoint}")
        ...
        export_path = os.path.join(models_dir, f'{model_name}.keras')
        export_info_path = os.path.join(run_info_dir, f'{model_name}.csv')
        save_checkpoint_path = os.path.join(ckpt_dir, f'{model_name}.keras')
        ...
        run_info = {
            'model': self.p.which_model,
            'timepoint': timepoint,
            'timestamp': timestamp,
            'model_timestamp': model_name,
        ...
        # Initialize the iterators before creating generators
        ...
        try:
            ...
            print(f"Test2 dataset created successfully")
        except Exception as e:
            print(f"Error creating test2 dataset: {e}")
            return
        ...
        print(f'Dataset lengths - Train: {train_length}, Val: {val_length}, Test: {test_length}')

        # Check minimum dataset requirements
        min_samples = self.p.BATCH_SIZE * 2  # At least 2 batches
        if train_length < min_samples:
            print(f"ERROR: Insufficient training data for {timepoint}. Need at least {min_samples}, got {train_length}")
            if self.use_wandb:
                wandb.finish()
            return

        if val_length < self.p.BATCH_SIZE:
            print(f"ERROR: Insufficient validation data for {timepoint}. Need at least {self.p.BATCH_SIZE}, got {val_length}")
            if self.use_wandb:
                wandb.finish()
            return
        ...
        cp_callback = tf.keras.callbacks.ModelCheckpoint(
            save_checkpoint_path, monitor='val_accuracy', verbose=1,
        ...
        tb_callback = tf.keras.callbacks.TensorBoard(
            log_dir=os.path.join(tb_log_dir, self.p.which_model),
            update_freq='epoch'
        )
        ...
        # Train model
        print(f"Training {timepoint} model...")
        history = model.fit(
            train_gen,
            steps_per_epoch=int(train_length // self.p.BATCH_SIZE),
            epochs=self.p.EPOCHS,
            validation_data=val_gen,
            validation_steps=int(val_length // self.p.BATCH_SIZE),
            callbacks=callbacks
        )

        # Evaluate model
        print(f'Evaluating {timepoint} model...')
        ...
        print(f'Saving {timepoint} model to {export_path}')
        model.save(export_path)

        print(f'{timepoint} Results:')
        print(f'  Train Accuracy: {run_info["train_accuracy"]:.4f}')
        print(f'  Val Accuracy: {run_info["val_accuracy"]:.4f}')
        print(f'  Test Accuracy: {run_info["test_accuracy"]:.4f}')
