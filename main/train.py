...
class Train:
    def __init__(self, parent_dir, param_dict=None, preprocess_tfrecs=False, use_wandb=True, log_artifact=True):
    ...
    def run(self, class_dirs, balance_method='cutoff', split_method='percentage',
        ...
        self.train()
        self._write_deploy_cmd(class_dirs)

    def _write_deploy_cmd(self, class_dirs):
        """Write a ready-to-run deploy_cmd.sh using the just-trained model and class mapping."""
        import json
        deploy_parent = os.path.join(self.parent_dir, 'deploy_results')
        # Build --class_dirs args: use training dirs as a reference (replace with val dirs before running)
        class_dirs_args = ' '.join(
            f'{lbl}:{d}' for lbl, dirs in enumerate(class_dirs) for d in dirs[:1]
        )
        script = (
            '#!/bin/bash\n'
            '# Auto-generated by train.py after training.\n'
            '# IMPORTANT: Replace each <val_dir> with the actual validation/test image directory.\n'
            '#            Training dirs are shown as reference for the correct label mapping.\n\n'
            'export PYTHONPATH=/home/shijiewang/Documents/GEDI-ORDER\n\n'
            f'MODEL_PATH="{getattr(self, "export_path", "<model_path>")}"\n'
            f'PARENT_DIR="{deploy_parent}"\n\n'
            '# Class label mapping (same order as training):\n'
        )
        for lbl, dirs in enumerate(class_dirs):
            script += f'#   {lbl} = {dirs[0]}\n'
        script += (
            '\n'
            'CLASS_DIRS=(\n'
        ...
        for lbl, dirs in enumerate(class_dirs):
            script += f'    "{lbl}:{dirs[0]}"  # TODO: replace with val dir\n'
        script += (
            ')\n\n'
            'mkdir -p "$PARENT_DIR"\n\n'
            'python /home/shijiewang/Documents/GEDI-ORDER/deploy/deploy.py \\\n'
            '    --parent "$PARENT_DIR" \\\n'
            '    --resdir "$PARENT_DIR" \\\n'
            '    --model_path "$MODEL_PATH" \\\n'
            f'    --which_model {self.p.which_model} \\\n'
            '    --preprocess_tfrecs 1 \\\n'
            '    --use_gedi_cnn 0 \\\n'
            '    --class_dirs "${CLASS_DIRS[@]}"\n'
        )
        cmd_path = os.path.join(self.parent_dir, 'deploy_cmd.sh')
        ...
        print(f'\nDeploy command saved to: {cmd_path}')
        print('Edit CLASS_DIRS in that file to point to validation image directories, then run it.')
    ...
    def generate_tfrecs(self, class_dirs, balance_method='cutoff', split_method='percentage',
        ...
        # Cap images per class (set via --max_per_class, 0 = no cap).
        # Use random.sample so that when multiple dirs are combined for one class,
        # all dirs contribute fairly instead of alphabetically-first dirs filling the cap.
        random.seed(42)
        if self.p.max_per_class > 0:
            all_class_imgs = [
                random.sample(imgs, min(self.p.max_per_class, len(imgs)))
                for imgs in all_class_imgs
            ]
        ...
        # Save label → directory mapping so you always know which class is which
        import json
        class_map = {i: dirs for i, dirs in enumerate(class_dirs)}
        class_map_path = os.path.join(tfrec_dir, 'class_map.json')
        with open(class_map_path, 'w') as f:
            json.dump(class_map, f, indent=2)
        print('Class label mapping:')
        for lbl, dirs in class_map.items():
            print(f'  {lbl}: {dirs}')
        print(f'Saved class_map.json to {class_map_path}')
    ...
    def train(self):
        ...
        self.export_path = export_path  # expose for deploy_cmd.sh generation

        # Log model to wandb
        if self.use_wandb and self.log_artifact:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datadir', action="store",
                        ...
                        dest='datadir')
    # Multi-class format (matches deploy.sh):
    #   --class_dirs 0:/path/c0 1:/path/c1 2:/path/c2
    # Multiple dirs per class are allowed: 0:/path/a 0:/path/b 1:/path/c
    parser.add_argument('--class_dirs', nargs='+', default=None, dest='class_dirs_labeled',
                        help='Class dirs as label:dir pairs, e.g. 0:/path/c0 1:/path/c1 2:/path/c2')
    # Legacy: repeat --class_dir once per class (label = order, 0-indexed)
    parser.add_argument('--class_dir', nargs='+', action='append', dest='class_dirs_legacy',
                        help='[legacy] Directories for one class; repeat for each class.')
    ...
    parser.add_argument('--use_wandb', type=int, action="store", default=True,
                        ...
                        dest="use_wandb")
    parser.add_argument('--log_artifact', type=int, action="store", default=1,
                        help='Upload model file as W&B artifact (0 = skip, faster runs)',
                        dest="log_artifact")
    ...
    parser.add_argument('--learning_rate', default=0.0005, type=float,
                        help='Learning rate')
    parser.add_argument('--max_per_class', default=10000, type=int,
                        help='Max images per class before balancing (0 = no cap)',
                        dest='max_per_class')
    ...
    # Resolve class_dirs: prefer --class_dirs (label:dir), then --class_dir (legacy), then --pos/neg_dir
    if args.class_dirs_labeled:
        # Parse "label:dir" pairs, group by label, sort by label
        parsed = {}
        for item in args.class_dirs_labeled:
            lbl_str, d = item.split(':', 1)
            parsed.setdefault(int(lbl_str), []).append(d)
        class_dirs = [parsed[i] for i in sorted(parsed.keys())]
    elif args.class_dirs_legacy:
        class_dirs = args.class_dirs_legacy
    elif args.pos_dir and args.neg_dir:
        class_dirs = [args.neg_dir, args.pos_dir]  # neg=0, pos=1
    else:
        raise ValueError('Provide --class_dirs 0:/path 1:/path ... (or legacy --class_dir / --pos_dir/--neg_dir)')
    ...
    Tr = Train(parent_dir=args.datadir, param_dict=param_dict, preprocess_tfrecs=args.preprocess_tfrecs,
               use_wandb=args.use_wandb, log_artifact=bool(args.log_artifact))
    if args.retrain:
